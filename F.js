...
if(process.argv.length < 6){
	console.log('error: not found argv'+process.argv.length+'\nnode F <input image path> <psf.json> <output image path> <parseInt mode(0 or 1 or 2)>');
	process.exit(1);
}
...
const img_org_array_vec = ps[ps.mode[parseInt_index]](tm.timer(() => D.dots(P_inv, img_array_vec)));
